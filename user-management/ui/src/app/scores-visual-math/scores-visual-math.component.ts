...
@Component({
  selector: 'app-scores-visual-math',
  templateUrl: './scores-visual-math.component.html',
  styleUrls: ['./scores-visual-math.component.css']
})
export class ScoresVisualMathComponent implements OnInit {
  ...
  createHistogram() {
    ...
    this.scores.sort(compareMath);

    if (this.parentNativeElement !== null) {
      let topTenMathScores = this.scores.slice(0, 10);
      //console.log("parentNativeElement not null");
      colors = ['#BF3500', '#C30037', '#C700AA', '#7400CB', '#0000CF',
                '#0077D4', '	#00D8BB', '#00DC41', '#3DE000', '#C1E500'];
      
      var schoolNames = topTenMathScores.map(score => score.school_name);
      ...
      var maxMathScore = d3.max(topTenMathScores, function(d) { return parseInt(d.sat_math_avg_score)});
      var mathScoreScale = d3.scaleLinear().domain([0, maxMathScore]).range([0, width]);
      var barHolder = svg.append('g').classed('bar-holder', true);

      var xAxis = d3.axisBottom(mathScoreScale).tickSizeOuter(0);
      ...
      var bars = barHolder.selectAll('rect.bar')
                .data(topTenMathScores)
                .enter()
                .append('rect')
                .classed('bar', true)
                .attr('x', 0)
                .attr('width', function(d) {
                  return mathScoreScale(parseInt(d.sat_math_avg_score));
                })
                .transition()
                .duration(1500)
                .attr('y', function(d) {
        ...
        svg.append("text")
                ...
                .text("Top 10 SAT Math Average Scores");
        */
    }
  }
}
...
function compareMath(a, b) {
  const mathA = parseInt(a.sat_math_avg_score);
  const mathB = parseInt(b.sat_math_avg_score);
  let comparison = 0;
  if (mathA < mathB) {
    comparison = 1;
  } else if (mathA > mathB) {
